...
SUBROUTINE broyden(F_EXO,fvec,x,n,df,intlj,reevalj,check,maxstp,TOLF)
...
REAL(prec), PARAMETER :: EPS=epsilon(x),TOLMIN=1.0e-10,TOLX=1.0e-10
real(prec)::TOLF
REAL(prec) :: f,fold
...
!			print*, ' '
	...
	call lnsrch(xold,fold,g,p,x,n,f,fvec,check,F_EXO,maxstp)
	...
	if (check) then													! True if line search failed to find a new x
		...
		if (restrt .or. maxval(abs(g(:))*max(abs(x(:)), 1.0)/max(f,0.5*n)) < TOLMIN) then		
			print*,'failure  broyden ' ,xc,hc,it
			return
		endif
...
!			print*,'finished, iteration step too small' ,xc,hc,it
			return				
		endif
	endif
end do
check=.true.
print*,'MAXITS exceeded in broyden' ,xc,hc,it

end subroutine broyden
! ---------------------------------------------
! ---------------------------------------------
SUBROUTINE lnsrch(xold,fold,g,p,x,n,f,fvec,check,F_EXO,maxstp)
...
! ---------------------------------------------
subroutine fminbr(F_EXO,f,fvec,x,n)
...
! ---------------------------------------------
SUBROUTINE fdjac(F_EXO,x,fvec,df)
