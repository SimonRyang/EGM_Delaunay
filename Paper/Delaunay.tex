...
\end{align*}
with $p = \frac{a_{X} - a_{A}} {a_{B} - a_{A}}$ and $q = \frac{h_{X} - h_{A}} {h_{C} - h_{A}}$, cf.~\citeN{judd98}.
...
\begin{enumerate}
	\item To initialize ENDGM predefine two grids, one for gross savings $s$, $G^{s} \equiv \left\{s^{1},s^{2},...,s^{K}\right\}$ which is defined as $s = a_{t} + w h_{t} - c_{t} - i_{t} = \frac{a_{t+1}} {R}$ and one for human capital $z$, $G^{z} \equiv \left\{z^{1},z^{2},...,z^{J}\right\}$ which is defined as $z = h_{t} + f\left(i_{t}\right) = \frac{h_{t+1}} {1 - \delta}$ and form $\mathcal{G}^{s,z} = \mathcal{G}^{s} \otimes \mathcal{G}^{z}$

	\item Define $\mathcal{G}^{a,h} = \mathcal{G}^{a} \otimes \mathcal{G}^{h}$ for $T$. In period $T$, as in EXGM, 
	\begin{align*}
	c_{T}\left(\cdot,\cdot\right)  &  = a_{T}^{k,j} + w h_{T}^{k,j}  \\
	...
	\begin{enumerate}
		...
		a_{t}^{k,j}  &  = s^{k} - w h_{t}^{k,j} + c_{t}^{k,j} + i_{t}^{k,j}  \text{.}
		\end{align*}
...
\begin{threeparttable}
	\begin{tabular}{L{0.1cm}C{2.5cm}C{2cm}C{2cm}C{3cm}C{3cm}}
	  \toprule
		& & \multicolumn{2}{c}{Speed} & \multicolumn{2}{c}{Euler Equation Error } \\ 
		\multicolumn{2}{L{3cm}}{Number of Grid- points for $(a,h)$} & Seconds & Relative to ENDGM & Maximum for  $c\ ;\ i$ & Average for \newline $c\ ;\ i$  \\ \hline \noalign{\smallskip}
		\multicolumn{6}{l}{ENDGM}  \\
